...
import {
  Plus, Pencil, Trash2, Upload, X, Layers, Eye, EyeOff,
...
import { fetchSubcategories, createSubcategory, updateSubcategory, deleteSubcategory } from '../redux/slices/subcategorySlice.js'
...
const SubcategoriesPage = () => {
  ...
  const fetchItems = useCallback(async (showRefreshIndicator = false) => {
    try {
      ...
      const { data } = await api.get('/admin/subcategories', { params })
      setItems(data.subcategories || [])
      setTotalPages(data.totalPages || 1)
      setTotal(data.total || 0)
    } catch (err) {
      console.error('Failed to fetch subcategories:', err)
      showToast(err.response?.data?.message || 'Failed to fetch subcategories', 'error')
    } finally {
      setIsLoading(false)
  ...
  const validateForm = () => {
    ...
    if (!form.category) errors.category = 'Parent category is required'
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/admin/subcategories/${deleteTarget._id}`)
  ...
  const handleToggleStatus = async () => {
    ...
    setIsConfirming(true)
    try {
      await api.patch(`/admin/subcategories/${toggleTarget._id}/status`)
      showToast(
  ...
  const quickToggle = async (item) => {
    try {
      await api.patch(`/admin/subcategories/${item._id}/status`)
  ...
  return (
    <motion.div className="space-y-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
      ...
      <PageHeader
        title="Subcategories"
        subtitle="Manage your product subcategories"
        breadcrumbs={[
      ...
      >
        ...
        >
          ...
          <div className="relative flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Total Subcategories</p>
        ...
        >
          ...
          <div className="relative flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-info mt-1">{totalProducts}</p>
            </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
            <span className="text-sm text-gray-500 dark:text-gray-400">
              <span className="font-semibold text-gray-700 dark:text-gray-300">{total}</span> subcategories
            </span>
            <button
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <p className="text-sm text-gray-500 dark:text-gray-400 max-w-[220px] truncate">
                          {item.description || <span className="italic text-gray-300 dark:text-gray-600">No description</span>}
                        </p>
                      </td>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
                    <div className="text-gray-500 dark:text-gray-400">
                      ...
                      <Badge variant="primary" size="sm">{item.category?.name || 'Uncategorized'}</Badge>
                    </div>
                    <div className="text-gray-500 dark:text-gray-400">
            ...
            <div className="px-6 py-4 border-t border-light-border dark:border-dark-border bg-gray-50/50 dark:bg-dark/30">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  ...
                  {' · '}
                  <span className="font-medium text-gray-700 dark:text-gray-300">{total}</span> total subcategories
                </p>
      ...
      <Modal
        ...
        title={
          <div className="flex items-center gap-3">
            ...
            <span>{editingId ? 'Edit Subcategory' : 'Create Subcategory'}</span>
          </div>
        }
        size="lg"
      >
                ...
                >
                  ...
                  <div className="text-center">
                    <p className="text-sm font-medium text-gray-600 dark:text-gray-400">
                      {isDragOver ? 'Drop image here' : 'Upload subcategory image'}
                    </p>
      ...
      <ConfirmationDialog
        ...
        message={
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-400">This action cannot be undone. Products linked to this subcategory will need reassignment.</p>
          </div>
        }
        confirmText="Delete"
        type="danger"
      ...
      <ConfirmationDialog
        ...
        title={toggleTarget?.isActive ? 'Deactivate Subcategory' : 'Activate Subcategory'}
        message={
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-400">
              {toggleTarget?.isActive
                ? 'Deactivating will hide this subcategory and its products from customers.'
                : 'Activating will make this subcategory and its products visible to customers.'}
            </p>
          </div>
        }
