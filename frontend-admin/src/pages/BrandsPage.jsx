...
const BrandsPage = () => {
  ...
  const fetchItems = useCallback(async (showRefreshIndicator = false) => {
    try {
      ...
      showToast(err.response?.data?.message || 'Failed to fetch brands', 'error')
    } finally {
      setIsLoading(false)
  ...
  // ── Fetch subcategories for cascade dropdown ──
  ...
  const totalCategories = new Set(items.map(b => b.category?._id).filter(Boolean)).size
  ...
  const openEditModal = (item) => {
    ...
    // Fetch subcategories for the brand's category
  ...
  const handleToggleStatus = async () => {
    ...
    setIsConfirming(true)
    try {
      await api.patch(`/admin/brands/${toggleTarget._id}/status`)
      showToast(
  ...
  const quickToggle = async (item) => {
    try {
      await api.patch(`/admin/brands/${item._id}/status`)
        ...
        >
          ...
          <div className="relative flex items-center justify-between">
            ...
            <div className="p-3 rounded-xl bg-primary/10 dark:bg-primary/20">
              <Award size={22} className="text-primary dark:text-primary-light" />
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="relative flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-info mt-1">{totalCategories}</p>
            </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
            <span className="text-sm text-gray-500 dark:text-gray-400">
              <span className="font-semibold text-gray-700 dark:text-gray-300">{total}</span> brands
            </span>
            <button
                  ...
                  {items.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        <Badge variant="outline" size="sm">{item.order ?? 0}</Badge>
                      </td>
            ...
            <div className="px-6 py-4 border-t border-light-border dark:border-dark-border bg-gray-50/50 dark:bg-dark/30">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  ...
                  {' · '}
                  <span className="font-medium text-gray-700 dark:text-gray-300">{total}</span> total brands
                </p>
      ...
      <Modal
        ...
        title={
          <div className="flex items-center gap-3">
            ...
            <span>{editingId ? 'Edit Brand' : 'Create Brand'}</span>
          </div>
        }
        size="lg"
      >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  value={form.subcategory ? (subcategories.find(s => s._id === form.subcategory)?.category?._id || '') : ''}
              ...
              <div>
                ...
                <select
                  ...
                  className={`select ${formErrors.subcategory ? 'border-danger focus:ring-danger' : ''}`}
                >
                ...
                >
                  ...
                  <div className="text-center">
                    <p className="text-sm font-medium text-gray-600 dark:text-gray-400">
                      {isDragOver ? 'Drop logo here' : 'Upload brand logo'}
                    </p>
      ...
      <ConfirmationDialog
        ...
        message={
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-400">This action cannot be undone. Products linked to this brand will need reassignment.</p>
          </div>
        }
        confirmText="Delete"
        type="danger"
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-400">
              {toggleTarget?.isActive
                ? 'Deactivating will hide this brand and its products from customers.'
                : 'Activating will make this brand and its products visible to customers.'}
            </p>
          </div>
        }
